...
rcri = latpb*float(ncube-0.5)/2.0 - latpb*0.3
...
rmv = []
for j in range(int(ntot)):
    if (abs(xu[j])>rcri or abs(yu[j])>rcri or abs(zu[j])>rcri) and atype[j] == 2: nis += 1
    if (abs(xu[j])>rcri or abs(yu[j])>rcri or abs(zu[j])>rcri) and atype[j] == 2: rmv.append(j)
#nistheory = float(ncube*ncube)*6
#if nis != nistheory: print ('wrong # of surface I', nis, nistheory)

indi = []
for j in range(int(ntot)):
    if j not in rmv and atype[j] == 2: indi.append(j)
...
print ('Std. PbIPb angle = %lf' %(np.std(angle)))
