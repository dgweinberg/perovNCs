...
# how to use example
# python pbipb_ang_rec_dft.py prop.xyz 2 2 2
...
latcx = ins/float(nux)
rcrix = latpbx*float(nux)/2.0 - latpbx*0.3
rcriy = latpby*float(nuy)/2.0 - latpby*0.3
rcriz = latpbz*float(nuz)/2.0 - latpbz*0.3
...
rmv = []
for j in range(int(ntot)):
    #if (abs(xu[j])>rcrix or abs(yu[j])>rcriy or abs(zu[j])>rcriz) and atype[j] == 3: print (j)
    if (abs(xu[j])>rcrix or abs(yu[j])>rcriy or abs(zu[j])>rcriz) and atype[j] == 3: nis += 1
    if (abs(xu[j])>rcrix or abs(yu[j])>rcriy or abs(zu[j])>rcriz) and atype[j] == 3: rmv.append(j)
nistheory = float(nux*nuy + nux*nuz + nuy*nuz)*2
...
for i in range(len(indi)):
    ...
    for j in range(npb):
        ...
        if rr < latpbx*0.9: nchk.append(jpb)
