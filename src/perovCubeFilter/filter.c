...
void filtering(zomplex *psims,double *potl,double *ksqr,zomplex *an,double *zn,double *el,long_st ist,par_st par,nlc_st *nlc,long *nl,long tid,long jns)
{
  ...
  filter(psi,phi,psims,potl,ksqr,par,nlc,nl,an,zn,ist,planfw,planbw,fftwpsi,tid,jns); 
  normalize_all(psims,par.dv,ist.ms,ist.nspinngrid,1);
  
  /*** calculate and print the energy of the filtered states ***/
  energy_all(psi,phi,psims,potl,ksqr,eval,ist,par,nlc,nl,planfw,planbw,fftwpsi);
  ...
  pf = fopen(str , "w");
  fwrite (psims,sizeof(zomplex),ist.ms*ist.nspinngrid,pf);
  fclose(pf);
...
void filter(zomplex *psin,zomplex *psim1,zomplex *psims,double *potl,double *ksqr,par_st par,nlc_st *nlc,long *nl,zomplex *an,double *zn,long_st ist,fftw_plan_loc planfw,fftw_plan_loc planbw,fftw_complex *fftwpsi,long tid,long jns)
{
  ...
  for (pf = fopen(str , "w"), jc = 1; jc < ist.nc; jc++){
    ...
    hnorm(psim1,psin,potl,ksqr,par,nlc,nl,zn[jc-1],ist,planfw,planbw,fftwpsi);
